...
if [ -n "${GITHUB_TOKEN}" ]; then
    ...
    remote_repo="https://x-access-token:${GITHUB_TOKEN}@${GITHUB_DOMAIN:-"github.infra.cloudera.com"}/${GITHUB_REPOSITORY}.git"
elif [ -n "${PERSONAL_TOKEN}" ]; then
    print_info "setup with PERSONAL_TOKEN"
    remote_repo="https://x-access-token:${PERSONAL_TOKEN}@${GITHUB_DOMAIN:-"github.infra.cloudera.com"}/${GITHUB_REPOSITORY}.git"
else
    print_info "no token found; linting"
    exec -- mkdocs build --config-file "${CONFIG_FILE}" --strict
fi

# workaround, see https://github.infra.cloudera.com/actions/checkout/issues/766
...
if ! git config --get user.email; then
    git config --global user.email "${GITHUB_ACTOR}@users.noreply.${GITHUB_DOMAIN:-"github.infra.cloudera.com"}"
fi
